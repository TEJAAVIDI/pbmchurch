...
namespace PBMChurch.Controllers
{
    ...
    public class WishesController : Controller
    {
        ...
        public WishesController(AppDbContext context, IWebHostEnvironment environment)
        {
            _context = context;
        ...
        {
            var wishes = await _context.WishTemplates.OrderBy(w => w.Type).ThenByDescending(w => w.CreatedDate).ToListAsync();
            
            // Get or create today's Bible reading
            var today = DateTime.Today;
            var dailyReading = await GetOrCreateDailyReading(today);
            
            var bibleReading = new BibleReading
            {
                CurrentDay = dailyReading.DayOfYear,
                ChaptersPerDay = 3,
                StartChapter = 0,
                EndChapter = 0,
                ReadingDate = today,
            ...
            ViewBag.ReadingRange = dailyReading.ReadingRange;
            
            return View(wishes);
        }

        private async Task<DailyBibleReading> GetOrCreateDailyReading(DateTime date)
        {
            var existing = await _context.DailyBibleReadings
                .FirstOrDefaultAsync(d => d.ReadingDate == date.Date);
            
            var dayOfYear = date.DayOfYear;
            var readingRange = GetReadingForDay(dayOfYear);
            
            if (existing != null)
            {
                // Update existing record with correct reading
                existing.ReadingRange = readingRange;
                existing.DayOfYear = dayOfYear;
                
                // Also update the Bible template with correct reading
                await CreateOrUpdateBibleTemplate(readingRange);
            ...
            var newReading = new DailyBibleReading
            {
            ...
            _context.DailyBibleReadings.Add(newReading);
            
            // Create/Update Bible template with today's reading
            await CreateOrUpdateBibleTemplate(readingRange);
        ...
        private async Task CreateOrUpdateBibleTemplate(string readingRange)
        {
            var bibleTemplate = await _context.WishTemplates
                .FirstOrDefaultAsync(w => w.Type == "Bible");
            
            var formattedMessage = $"Today's Bible Reading: {readingRange}. May God's word guide and bless you today and forever.";
            
            if (bibleTemplate == null)
            {
                bibleTemplate = new WishTemplate
                {
                    Type = "Bible",
                ...
                _context.WishTemplates.Add(bibleTemplate);
            }
            else
            {
                bibleTemplate.MessageText = formattedMessage;
                bibleTemplate.ModifiedBy = "System";
                bibleTemplate.ModifiedDate = DateTime.Now;
            }
        }

        public IActionResult Create(string type = "Birthday")
        {
            var wish = new WishTemplate { Type = type };
            ViewBag.Types = new[] { "Birthday", "Anniversary", "Event", "Bible", "General" };
            return View(wish);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Create(WishTemplate wish, IFormFile? imageFile)
        {
            if (ModelState.IsValid)
            {
                ...
                if (imageFile != null && imageFile.Length > 0)
                {
                    using (var memoryStream = new MemoryStream())
                    {
                        ...
                        wish.ImageData = memoryStream.ToArray();
                        wish.ImageFileName = imageFile.FileName;
                        wish.ImageContentType = imageFile.ContentType;
                    }
                }

                wish.CreatedBy = GetCurrentAdminId();
                wish.CreatedDate = DateTime.Now;
                
                _context.WishTemplates.Add(wish);
                await _context.SaveChangesAsync();
                
                TempData["Success"] = $"{wish.Type} template created successfully!";
        ...
        {
            var wish = await _context.WishTemplates.FindAsync(id);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, WishTemplate wish, IFormFile? imageFile)
        {
            if (id != wish.Id) return NotFound();

            if (ModelState.IsValid)
            {
                try
                {
                    var existingWish = await _context.WishTemplates.FindAsync(id);
                    ...
                    if (imageFile != null && imageFile.Length > 0)
                    {
                        ...
                        {
                            await imageFile.CopyToAsync(memoryStream);
                            existingWish.ImageData = memoryStream.ToArray();
                            existingWish.ImageFileName = imageFile.FileName;
                            existingWish.ImageContentType = imageFile.ContentType;
                        }
                    }

                    existingWish.Type = wish.Type;
                    existingWish.MessageText = wish.MessageText;
                    existingWish.YouTubeUrl = wish.YouTubeUrl;
                    existingWish.IsActive = wish.IsActive;
                    existingWish.ModifiedBy = GetCurrentAdminId();
                    existingWish.ModifiedDate = DateTime.Now;
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            if (wish != null)
            {
                _context.WishTemplates.Remove(wish);
                await _context.SaveChangesAsync();
        ...
        [HttpPost]
        public async Task<IActionResult> SendWhatsAppMessage(int memberId, string messageType)
        {
            ...
            var template = await _context.WishTemplates
                .Where(w => w.Type == messageType && w.IsActive)
                .FirstOrDefaultAsync();

            if (template == null)
            {
                return Json(new { success = false, message = $"No {messageType} template found" });
            }

            var message = template.MessageText?.Replace("{Name}", member.Name) ?? $"Happy {messageType} {member.Name}!";
            var phoneNumber = member.Phone.Replace("+", "").Replace("-", "").Replace(" ", "");
            var whatsappUrl = $"https://wa.me/{phoneNumber}?text={Uri.EscapeDataString(message)}";

            return Json(new { success = true, url = whatsappUrl });
        }

        private async Task<List<Member>> GetTodaysBirthdays()
        {
            var today = DateTime.Today;
        ...
        {
            // Same reading plan as Dashboard - 3 chapters per day to complete Bible in a year
        ...
        {
            var wish = await _context.WishTemplates.FindAsync(id);
            if (wish?.ImageData != null)
            {
                return File(wish.ImageData, wish.ImageContentType ?? "image/jpeg");
            }
            return NotFound();
        }
    }
