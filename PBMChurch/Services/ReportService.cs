...
namespace PBMChurch.Services
{
    ...
    {
        Task<AttendanceReportViewModel> GetAttendanceReportAsync(ReportFilterViewModel filter);
        Task<FinancialReportViewModel> GetFinancialReportAsync(int? churchId, int year, int month);
    ...
    public class ReportService : IReportService
    {
        ...
        public async Task<AttendanceReportViewModel> GetAttendanceReportAsync(ReportFilterViewModel filter)
        {
            ...
            if (filter.ChurchId.HasValue)
                query = query.Where(a => a.ChurchId == filter.ChurchId.Value);
            ...
            {
                bool isPresent = filter.Status.Equals("Present", StringComparison.OrdinalIgnoreCase);
                query = query.Where(a => a.IsPresent == isPresent);
            }
            ...
            return new AttendanceReportViewModel
            {
                Filters = filter,
                ReportData = data,
                TotalPresent = data.Count(d => d.IsPresent),
                TotalAbsent = data.Count(d => !d.IsPresent),
                TotalRecords = data.Count
            };
        }

        public async Task<FinancialReportViewModel> GetFinancialReportAsync(int? churchId, int year, int month)
        {
            ...
            var incomeItems = await incomeQuery
                .Select(i => new IncomeReportItem
                {
                    IncomeId = i.IncomeId,
                    ChurchName = i.Church!.ChurchName,
                    Amount = i.Amount,
                    Source = i.Source,
            ...
            var expenseItems = await expenseQuery
                .Select(e => new ExpenseReportItem
                {
                    ExpenseId = e.ExpenseId,
                    ChurchName = e.Church!.ChurchName,
                    Amount = e.Amount,
                    Category = e.Category,
            ...
            return new FinancialReportViewModel
            {
                ...
                TotalExpense = expenseItems.Sum(e => e.Amount),
        ...
        public async Task<DashboardViewModel> GetDashboardDataAsync()
        {
            ...
            var presentToday = todayAttendance.Count(a => a.IsPresent);
            var absentToday = totalMembers - presentToday;

            // Church-wise attendance
            var churchWiseAttendance = await GetChurchWiseAttendanceAsync(today);

            // Multi-church attendance summary for today
            var todayDayOfWeek = today.DayOfWeek.ToString();
            var churchesWithServiceToday = await _context.Churches
                .Where(c => c.Status == "Active" && (c.MeetingDay1 == todayDayOfWeek || c.MeetingDay2 == todayDayOfWeek))
                .ToListAsync();

            var todayChurchAttendances = new List<TodayChurchAttendance>();
            foreach (var church in churchesWithServiceToday)
            {
                var stats = churchWiseAttendance.FirstOrDefault(c => c.ChurchId == church.ChurchId);
                if (stats != null)
                {
                    todayChurchAttendances.Add(new TodayChurchAttendance
                    {
            ...
            // Today's birthdays
            var todayBirthdays = await GetBirthdaysAsync(today, today);
            var birthdaysToday = todayBirthdays.Count;

            // Upcoming birthdays (next 7 days)
            var upcomingBirthdays = await GetBirthdaysAsync(today.AddDays(1), today.AddDays(7));

            // Financial data
            var monthIncome = await _context.Income
                .Where(i => i.IncomeDate.Year == today.Year && i.IncomeDate.Month == today.Month)
            ...
            // This week meetings
            var thisWeekMeetings = await GetThisWeekMeetingsAsync();

            // Bible Reading Info
            var bibleReading = GetTodayBibleReading();

            int presentForSelected = 0;
            int absentForSelected = 0;
            string selectedChurchName = string.Empty;
            if (todayDayOfWeek == DayOfWeek.Sunday.ToString())
            {
                selectedChurchName = "All Churches";
            }
            else if (churchesWithServiceToday.Count == 1)
            {
                var church = churchesWithServiceToday.First();
                var stats = todayChurchAttendances.FirstOrDefault(x => x.ChurchId == church.ChurchId);
                presentForSelected = stats?.PresentCount ?? 0;
                absentForSelected = stats?.AbsentCount ?? 0;
                selectedChurchName = church.ChurchName;
            }
            else if (churchesWithServiceToday.Count > 1)
            {
                presentForSelected = todayChurchAttendances.Sum(x => x.PresentCount);
                absentForSelected = todayChurchAttendances.Sum(x => x.AbsentCount);
                selectedChurchName = "All Churches";
            }
            return new DashboardViewModel
            {
                ...
                PresentTodayForSelectedChurch = presentForSelected,
                AbsentTodayForSelectedChurch = absentForSelected,
                SelectedChurchName = selectedChurchName,
                TodayChurchAttendances = todayChurchAttendances,
                ...
                ChurchWiseAttendance = churchWiseAttendance,
                ThisWeekMeetings = thisWeekMeetings,
                TodayBibleReading = bibleReading
            };
        }

        private async Task<List<BirthdayMember>> GetBirthdaysAsync(DateTime fromDate, DateTime toDate)
        {
            ...
            var birthdays = members
                ...
                .Select(m => new BirthdayMember
                {
                    ...
                    DaysUntilBirthday = (new DateTime(DateTime.Now.Year, m.DateOfBirth!.Value.Month, m.DateOfBirth.Value.Day) - DateTime.Today).Days,
        ...
        private async Task<List<ChurchAttendanceStats>> GetChurchWiseAttendanceAsync(DateTime date)
        {
            var churches = await _context.Churches
                .Where(c => c.Status == "Active")
                .ToListAsync();

            var stats = new List<ChurchAttendanceStats>();

            foreach (var church in churches)
            {
                var totalMembers = await _context.Members
                    .CountAsync(m => m.ChurchId == church.ChurchId && m.Status == "Active");

                var presentCount = await _context.Attendance
                    .CountAsync(a => a.ChurchId == church.ChurchId && a.AttendanceDate == date && a.IsPresent);

                stats.Add(new ChurchAttendanceStats
                {
                    ChurchId = church.ChurchId,
        ...
        private async Task<List<MeetingDay>> GetThisWeekMeetingsAsync()
        {
            var today = DateTime.Today;
            ...
            var meetings = new List<MeetingDay>();
            ...
            return meetings.OrderBy(m => m.Date).ToList();
        }

        private BibleReadingInfo GetTodayBibleReading()
        {
            var today = DateTime.Today;
            var dayOfYear = today.DayOfYear;
            var totalDays = DateTime.IsLeapYear(today.Year) ? 366 : 365;
            var progress = (decimal)dayOfYear / totalDays * 100;

            // Bible reading plan - approximately 3 chapters per day
            var readingText = GetReadingForDay(dayOfYear);

            return new BibleReadingInfo
            {
                DayOfYear = dayOfYear,
                Year = today.Year,
                ChaptersPerDay = 3, // Fixed to 3 chapters per day
        ...
        {
            // Proper daily Bible reading plan - 3 chapters per day to complete Bible in a year
